...
app.layout = html.Div(children=[html.H1('SpaceX Launch Rdecords Dashboard',
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # Use all rows, sum up class=1 per launch site
        fig = px.pie(
            spacex_df,
            values='class',
            names='Launch Site',
        ...
        # Filter dataframe for the selected site only
...
def get_scatter_chart(entered_site, payload_range):
    ...
    # Filter dataframe by the selected payload range first
    range_df = spacex_df[
